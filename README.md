...
Model Causal Graph (MCG)
A graph derived from the behavior of a trained machine learning model using concept-level interventions.
Human Causal Graph (HCG)
A graph derived from LLM-generated clinician-style causal reasoning over a fixed set of predefined concepts.
Cognitively Aligned Causal Graph (CACG)
A graph that combines the model-derived MCG and the human-derived HCG to produce explanations that are both model-faithful and clinically interpretable.
